...
<head>
    ...
    <style>
       ...
       .content-wrapper.expanded {
        ...
        /* NEW: Tom Select Custom Styling to match the rest of the form */
        .ts-control {
            border-radius: 0.5rem;
            padding: 0.6rem 1rem;
            border: 1px solid #dee2e6;
...
<body>
                ...
                <div class="row mb-4">
                    <div class="col-md-4 mb-3 mb-md-0">
                        <div class="card stat-card border-0 shadow-sm h-100"
                             ...
                             data-bs-target="#availableItemsModal"
                             title="Click to view available items">
                ...
                <div class="card border-0 shadow-sm rounded-4 mb-4">
                    ...
                    <div class="card-body p-4">
                        <form id="borrowForm" action="../ACTIONS/process_borrow.php" method="POST" class="row g-3 align-items-end">
                            ...
                            <div class="col-md-6">
                                <label class="form-label fw-bold small text-muted mb-2">Search Equipment Name</label>
                                <select name="item_id" id="equipmentSelect" class="form-select" placeholder="Type to search..." required>
                                    <option value=""></option> <?php foreach ($available_items_list as $item): ?>
    ...
    <div class="modal fade" id="availableItemsModal" tabindex="-1" aria-labelledby="availableItemsModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content border-0 shadow-lg" style="border-radius: 1rem;">
                <div class="modal-header bg-success text-white border-0 py-3" style="border-radius: 1rem 1rem 0 0;">
                    <h5 class="modal-title fw-bold" id="availableItemsModalLabel">
                        <i class="bi bi-check2-circle me-2"></i>Available Equipment
                    </h5>
                        ...
                        <table class="table table-hover align-middle mb-0">
                            <thead class="table-light position-sticky top-0">
                                <tr style="font-size: 0.85rem; text-transform: uppercase;">
                                    <th class="ps-4 py-3 border-bottom-0 text-muted">Equipment Name</th>
                                    <th class="py-3 border-bottom-0 text-muted">Serial Number</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php if (!empty($available_items_list)): ?>
                                    <?php foreach ($available_items_list as $item): ?>
                                        <tr>
                                            ...
                                            <td class="py-3 text-muted font-monospace small">
                                                <?php echo !empty($item['serial_number']) ? htmlspecialchars($item['serial_number']) : '<span class="text-black-50 fst-italic">N/A</span>'; ?>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php else: ?>
                ...
                <div class="modal-footer bg-light border-top-0 py-2 px-3" style="border-radius: 0 0 1rem 1rem;">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // 1. Initialize Tom Select (Makes the dropdown searchable)
            ...
            if (borrowForm) {
                borrowForm.addEventListener('submit', function(e) {
                    ...
                    const itemSelect = document.getElementById('equipmentSelect');
                    const itemName = itemSelect.options[itemSelect.selectedIndex].text; // Grab the visible text

                    Swal.fire({
                        title: 'Process Borrowing?',
                        html: `Assign Equipment <strong>${itemName}</strong> to Student <strong>${studentId}</strong>?`,
                        icon: 'question',
                        showCancelButton: true,
                        confirmButtonColor: '#3a5a40',
    ...
    <?php if (isset($_GET['status'])): ?>
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // Clear the status parameter from the URL so it doesn't pop up again on refresh
