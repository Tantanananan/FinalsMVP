...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // We fetch the user regardless of status first, so we can give them the correct penalty message
    ...
    if ($user && password_verify($password, $user['password'])) {
        ...
        // 1. Check for 4th Strike (Permanent Ban)
        if ($user['status'] === 'Archived') {
            $error = "Account Banned: Your account has been permanently archived due to 4 equipment strikes. Please visit the MIS office.";
        } 
        
        // 2. Check for Active Timeouts (1st, 2nd, 3rd Strike)
        elseif (!empty($user['penalty_end_date']) && strtotime($user['penalty_end_date']) > time()) {
            $formatted_date = date('F j, Y - g:i A', strtotime($user['penalty_end_date']));
            $error = "Account Suspended: You cannot log in until " . $formatted_date . " due to an equipment penalty.";
        } 
        
        // 3. General check in case account was deactivated manually for other reasons
        elseif ($user['status'] !== 'Active' && $user['status'] != 1) {
