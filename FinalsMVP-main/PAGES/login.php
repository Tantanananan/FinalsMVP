...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Fetch the user including the new timeout_until column
    ...
    if ($user && password_verify($password, $user['password'])) {
        ...
        // --- TIMEOUT CHECK ADDED HERE ---
        // If timeout_until has a date, AND that date is in the future, block them.
        if (!empty($user['timeout_until']) && strtotime($user['timeout_until']) > time()) {
            ...
            // No timeout, or timeout has passed. Proceed with login!
