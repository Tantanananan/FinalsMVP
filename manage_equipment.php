...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_equipment'])) {
    ...
    if ($stmt = $mysql->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            $message = "<script>document.addEventListener('DOMContentLoaded', function() { Swal.fire('Deleted!', 'Equipment removed.', 'success'); });</script>";
        } else {
            $error_msg = addslashes($stmt->error);
            $message = "<script>document.addEventListener('DOMContentLoaded', function() { Swal.fire('Error', 'Failed to delete: {$error_msg}', 'error'); });</script>";
        }
        $stmt->close();
    }
}
...
// FETCH TABLE
$query = "SELECT item_id, item_name, serial_Number, status FROM items ORDER BY item_id DESC";
...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div>
            <a href="adminDashboard.php" class="btn btn-outline-secondary me-2">Back to Dashboard</a>
